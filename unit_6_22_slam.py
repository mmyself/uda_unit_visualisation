...
class robot:
    ...
    def __repr__(self):
        mtpl.scatter(self.x,self.y, c='b', s=200.0, marker='o')
...
def make_data(N, num_landmarks, world_size, measurement_range, motion_noise, 
    ...
    mtpl.scatter([p[0] for p in r.landmarks],[p[1] for p in r.landmarks], c='r', s=200.0, marker='o')
    print r
...
def print_result(N, num_landmarks, result):
    ...
    for i in range(N):
        mtpl.scatter(result.value[2*i],result.value[2*i+1], c='y', s=20.0, marker='o', label='Estimated Pose(s)')
        print '    ['+ ', '.join('%.3f'%x for x in result.value[2*i]) + ', ' \
            + ', '.join('%.3f'%x for x in result.value[2*i+1]) +']'
    mtpl.scatter(result.value[2*i],result.value[2*i+1], c='g', s=100.0, marker='o')
    print
    print 'Estimated Landmarks:'
    for i in range(num_landmarks):
        mtpl.scatter(result.value[2*(N+i)],result.value[2*(N+i)+1], c='c', s=100.0, marker='o')
...
def slam2(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    def add_constraint(init_index, final_index, dist, confidence = 1.):

        # s(final) - s(init) = dist
        omega.value[init_index][init_index] += confidence
        omega.value[init_index][final_index] += -confidence
        xi.value[init_index][0] += -dist[0]*confidence
        xi.value[init_index][1] += -dist[1]*confidence
        omega.value[final_index][init_index] += -confidence
        omega.value[final_index][final_index] += confidence
        xi.value[final_index][0] += dist[0]*confidence
        xi.value[final_index][1] += dist[1]*confidence
    ...
    for d in range(len(data)):
        # landmarks measured before moving to pose d+1
        for landmark in data[d][0]:
            add_constraint(d, N + landmark[0], landmark[1:], 1./measurement_noise)

        # data has one fewer element than the number of poses
        # data[d][1] holds the distance from pose d to pose d+1
        add_constraint(d, d+1, data[d][1], 1./motion_noise)

    # compute mu
    mu = omega.inverse()*xi

    mu_interlace = []
    for coord_pair in mu.value:
        mu_interlace.append([coord_pair[0]])
        mu_interlace.append([coord_pair[1]])

    return matrix(mu_interlace)
